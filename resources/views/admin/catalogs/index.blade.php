...
@section('content')
    <div class="container-fluid">
        ...
        <form action="{{ route('admin.catalogs.multipleDelete') }}" method="POST" id="multiple-delete">
            @csrf
            <div class="row justify-content-center">
                <div class="col-md-12">
                    <div class="card">
                        <div class="card-header">
                            <h2 class="mb-0 float-start">{{ __('Catalogs') }}</h2>
                            <button type="submit" class="btn btn-danger float-end ms-2">
                                <i class="fa-solid fa-trash"></i> Delete Selected
                            </button>
                            <a href="{{ route('admin.catalogs.create') }}" class="btn btn-success float-end">
                                    ...
                                    <tbody id="sortable">
                                        @foreach ($catalogs as $index => $item)
                                            <tr data-id="{{ $item->id }}">
                                                ...
                                                <td>
                                                    <a href="{{ route('admin.catalogs.edit', $item->id) }}"
                                                        class="btn btn-primary">
                                                        <i class="fa-solid fa-pen-to-square"></i>
                                                    </a>
                                                    <a class="btn btn-danger delete-confirm"
                                                        data-url="{{ route('admin.catalogs.delete', $item->id) }}">
        ...
        $(function() {
            $('#sortable').sortable({
                update: function(event, ui) {
                    ...
                    $.ajax({
                        url: '{{ route('admin.catalogs.reorder') }}',
                        type: 'POST',
                        data: {
                            _token: '{{ csrf_token() }}',
                            order: order

...
@section('content')
    <div class="container-fluid">
        ...
        <form action="{{ route('admin.galleries.multipleDelete') }}" method="POST" id="multiple-delete">
            @csrf
            <div class="row justify-content-center">
                <div class="col-md-12">
                    <div class="card">
                        <div class="card-header">
                            <h2 class="mb-0 float-start">{{ __('Gallery Images') }}</h2>
                            <button type="submit" class="btn btn-danger float-end ms-2">
                                <i class="fa-solid fa-trash"></i> Delete Selected
                            </button>
                            <a href="{{ route('admin.galleries.create') }}" class="btn btn-success float-end">
                                            ...
                                            <tr data-id="{{ $item->id }}">
                                                ...
                                                <td>
                                                    ...
                                                    <a class="btn btn-danger delete-confirm"
                                                        data-url="{{ route('admin.galleries.delete', $item->id) }}">
        ...
        $(function() {
            $('#sortable').sortable({
                update: function(event, ui) {
                    ...
                    $.ajax({
                        url: '{{ route('admin.galleries.reorder') }}',
                        type: 'POST',
                        data: {
                            _token: '{{ csrf_token() }}',
                            order: order

...
@section('content')
    <div class="container-fluid">
        ...
        <form action="{{ route('admin.sizes.multipleDelete') }}" method="POST" id="multiple-delete">
            @csrf
            <div class="row justify-content-center">
                <div class="col-md-12">
                    <div class="card">
                        <div class="card-header">
                            <h2 class="mb-0 float-start">{{ __('Sizes') }}</h2>
                            <button type="submit" class="btn btn-danger float-end ms-2">
                                <i class="fa-solid fa-trash"></i> Delete Selected
                            </button>
                            <a href="{{ route('admin.sizes.create') }}" class="btn btn-success float-end">
                                        ...
                                        @foreach ($sizes as $item)
                                            <tr>
                                                <th scope="row">
                                                    ...
                                                    {{ $loop->iteration + ($sizes->firstItem() - 1) }}
                                                </th>
                                                <td>{{ $item->name }}</td>
                                                <td>{{ $item->code }}</td>
                                                <td>
